...
final class MarginListTable extends WP_List_Table {
	...
	public function no_items(): void {
		esc_html_e(
			'No coupon was used in this period.',
	...
	private function code_cell( CouponMargin $margin ): string {
		...
		return sprintf(
			'<strong>%s</strong>',
			esc_html( wp_specialchars_decode( $margin->code, ENT_QUOTES ) )
		);
	}
	...
	private function cost_cell( CouponMargin $margin ): string {
		if ( CostCoverage::NONE === $margin->coverage() ) {
			return sprintf(
				'<span class="dfxcaaw-unknown">%s</span>',
				esc_html__( 'Not known', 'coupon-audit-and-analytics-for-woocommerce' )
			);
		}
		...
		return $cost . sprintf(
			'<br /><span class="dfxcaaw-coverage">%s</span>',
			esc_html(
				sprintf(
	...
	private function margin_cell( CouponMargin $margin ): string {
		...
		if ( null === $value ) {
			return sprintf(
				'<span class="dfxcaaw-unknown">%s</span>',
				esc_html__( 'Needs cost data', 'coupon-audit-and-analytics-for-woocommerce' )
			);
		}
		...
		$rendered = sprintf(
			'<strong class="%1$s">%2$s</strong>',
			$value->is_negative() ? 'dfxcaaw-negative' : '',
			$this->money( $value->amount, $margin->currency() )
		);

		if ( null !== $share ) {
			$rendered .= sprintf(
				' <span class="dfxcaaw-share">%s</span>',
				esc_html(
					sprintf(
		...
		if ( CostCoverage::PARTIAL === $margin->coverage() ) {
			$rendered .= sprintf(
				'<br /><span class="dfxcaaw-coverage">%s</span>',
				esc_html__( 'estimate', 'coupon-audit-and-analytics-for-woocommerce' )
			);
		}
