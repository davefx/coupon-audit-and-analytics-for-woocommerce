...
final class MarginPage {
	...
	public function __construct(
		private readonly MarginService $margins,
		private readonly MarginListTable $table,
	...
	public function render(): void {
		if ( ! current_user_can( InventoryPage::CAPABILITY ) ) {
			wp_die(
				esc_html__(
					'You do not have permission to view coupon margins.',
		...
		printf(
			...
			esc_html(
				sprintf(
					...
					__(
						'Still reading past orders — figures are complete up to %s and will fill in as the work continues.',
						'coupon-audit-and-analytics-for-woocommerce'
					),
					$cursor
				)
	...
	private function render_coverage_notice( array $margins ): void {
		...
		if ( 0 === $known ) {
			printf(
				...
				esc_html__(
					'No margin can be shown yet, because none of these orders has a cost recorded against it. Set product costs in WooCommerce, or in whichever cost-of-goods plugin your shop already uses, and the figures will appear.',
					'coupon-audit-and-analytics-for-woocommerce'
				)
			);
		...
		if ( $known < count( $margins ) ) {
			printf(
				'<div class="notice notice-info inline"><p>%s</p></div>',
				esc_html__(
					'Some coupons have no cost recorded against their orders, so no margin is shown for them. Margins marked as an estimate cover only part of their lines.',
					'coupon-audit-and-analytics-for-woocommerce'
				)
			);
		}
